...
export default class SelectBar extends React.Component {
	...
	animateAfterSelect(e) {
		...
		let startPoint = this.currentQuestAnswer,
			value,
			bigThis = this,
			diff,
			result,
		...
		this.animate({
			...
			draw: function(progress) {
				...
				bigThis.currentQuestAnswer = result;
				console.log(result);
			}
		});
	}
	...
	answerSelecting(e) {
		...
		let value = parseInt(e.currentTarget.value),
			diff = this.currentQuestAnswer - value;
		if (diff > 50 || diff < -50) {
		...
		return (
			<div>
				...
				<input
					...
					max="500"
					//defaultValue="100"
					defaultValue={this.currentQuestAnswer}
					//value={this.state.currentQuestAnswer}
					//value={this.currentQuestAnswer}
					className={styles["slider"]}
					id="myRange"
				...
				/>
				<div>Текущий ответ:{this.currentQuestAnswer}</div>
			</div>
		);
	}
}
